...
export default function ResidentSelector({ dispatch, selectedState }: ResidentSelectorProps) {
  const { primaryResident, involvedResidents, involvedCaregivers } = selectedState;

  const [primaryResidentOptions, setPrimaryResidentOptions] = useState<ResidentTag[]>([])
  const [involvedResidentOptions, setInvolvedResidentOptions] = useState<ResidentTag[]>([])
          ...
          <div className="flex gap-2 justify-start items-center">
            ...
            <Popover>
              ...
              <PopoverContent className="p-0 w-[300px]">
                <Command>
                  <CommandInput placeholder="Search resident..." onValueChange={handlePrimaryResidentSearch} />
                  <CommandList>
                    ...
                    <CommandGroup>
                      {primaryResidentOptions.map((resident) => (
                        <CommandItem
                          key={resident.id}
                          onSelect={() => dispatch({ type: "SET_PRIMARY_RESIDENT", payload: resident })}>
                          {resident.name}
                        </CommandItem>
                      ))}
                    </CommandGroup>
          ...
          </div>
          {
            primaryResident ? (
              <Badge variant="secondary" className="font-medium">
                ...
                <X className="h-3 w-3 cursor-pointer" onClick={() => dispatch({ type: "UNSET_PRIMARY_RESIDENT", payload: primaryResident?.id })} />
              </Badge>
            ) : null
          }
        </div>
        ...
        <div className="">
          <div className="flex items-center gap-2 pb-2">
            ...
            <Popover>
              ...
              <PopoverContent className="p-0 w-[300px]">
                <Command>
                  <CommandInput placeholder="Search residents..." onValueChange={handleInvolvedResidentSearch} />
                  <CommandList>
                    <CommandEmpty>No resident found.</CommandEmpty>
                    <CommandGroup>
                      {involvedResidentOptions
                        .filter((resident) => !(resident.name === primaryResident?.name))
                        .map((resident) => (
                          <CommandItem
                            key={resident.id}
                            onSelect={() =>
                              dispatch({
                                type: involvedResidents.some((r) => r.id === resident.id)
                          ...
                          >
                            <Check className={cn("mr-2 h-4 w-4", involvedResidents.includes(resident) ? "opacity-100" : "opacity-0")} />
                            {resident.name}
                          </CommandItem>
                        ))}
                    </CommandGroup>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {involvedResidents.map((resident) => {
              return resident ? (
                <Badge key={resident.id} variant="secondary" className="font-medium">
                  {resident.name}
                  <X className="h-3 w-3 cursor-pointer" onClick={() => dispatch({ type: "REMOVE_INVOLVED_RESIDENT", payload: resident.id })} />
                </Badge>
              ) : null
            })}
          </div>
        ...
        <div>
          <div className="flex items-center gap-2 pb-2">
            ...
            <Popover>
              ...
              <PopoverContent className="p-0 w-[300px]">
                <Command>
                  <CommandInput placeholder="Search caregivers..." onValueChange={handleCaregiverSearch} />
                  <CommandList>
                    ...
                    <CommandGroup>
                      {caregiverOptions.map((caregiver) => (
                        <CommandItem
                          ...
                          onSelect={() =>
                            dispatch({
                              type: involvedCaregivers.some((c) => c.id === caregiver.id)
                        ...
                        >
                          <Check className={cn("mr-2 h-4 w-4", involvedCaregivers.includes(caregiver) ? "opacity-100" : "opacity-0")} />
                          {caregiver.name}
                        </CommandItem>
                      ))}
                    </CommandGroup>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {involvedCaregivers.map((caregiver) => {
              return caregiver ? (
                <Badge key={caregiver.id} variant="secondary" className="font-medium">
                  {caregiver.name}
                  <X className="h-3 w-3 cursor-pointer" onClick={() => dispatch({ type: "REMOVE_INVOLVED_CAREGIVER", payload: caregiver.id })} />
                </Badge>
              ) : null
            })}
          </div>
