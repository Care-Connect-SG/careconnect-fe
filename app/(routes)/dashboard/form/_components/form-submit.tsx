...
export default function FormSubmit({ form }: FormSubmitProps) {
    ...
    const handleInputChange = (form_element_id: string, input: any) => {
        setReport((prevReport) =>
            prevReport.map((section) =>
                section.form_element_id === form_element_id ? { ...section, input } : section
            )
        );
    };
    ...
    return (
        <div className="py-4 px-8">
            ...
            <Button disabled={isSubmitting} onClick={handleSubmit} className="bg-black hover:bg-gray-300 hover:text-black">
                Submit Form
            </Button>
        </div>
    );
